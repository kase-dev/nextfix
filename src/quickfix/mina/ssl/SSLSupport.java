...
public class SSLSupport {
    ...
    public static final String SETTING_ENABLE_PROTOCOLS = "EnabledProtocols";
    ...
    public static String getEnableProtocols(SessionSettings settings, SessionID sessionID) {
        String res = null;
        if (settings.isSetting(sessionID, SSLSupport.SETTING_ENABLE_PROTOCOLS)) {
            try {
                res = settings.getString(sessionID, SSLSupport.SETTING_ENABLE_PROTOCOLS);
            } catch (ConfigError | FieldConvertError ignored) {
            }
        }
        return res;
